...
db_path = f"{mol_sym.lower()}_pbe-def2svp_{num_mols}.db" if charge == 0 else f"{mol_sym.lower()}_pbe-def2svpd_{num_mols}_test.db"
preds_path = f"{mol_sym.lower()}_preds_test.npy"
...
def evaluate_dm_guess(mol, dm_pred: np.ndarray, name="system",
                      ...
                      baseline_guess="minao", auxbasis=auxbasis):
    ...
    mf_ref.init_guess = baseline_guess
    ...
    mf_pred.init_guess = baseline_guess
...
baseline_guess="huckel"
def evaluate_dataset_to_txt(dms_valid, rs_valid, chemical_symbols,
                            outfile=f"benchmarks_summary_{mol_sym.lower()}_vs_{baseline_guess}_test.txt",
                            basis=basis, unit=unit, charge=charge, spin=spin,
                            baseline_guess=baseline_guess, print_each=True):
    if basis is None:
