...
class NeuralNetwork(nn.Module):
    def __init__(self, 
            ...
            num_residual_pn      = 1, #number of residual blocks applied to output atomic features for constructing pair features (neighboring atoms)x
            ...
            restricted_scheme = True,
            system_net_charge = 0    ): 
        super(NeuralNetwork, self).__init__()

        #variables to control the flow of the forward graph (calculate density_matrix/core_hamiltonian/overlap_matrix/energy/forces?)
        self.calculate_density_matrix = True
        ...
        def check_net_charge():
            try:
                net_charge = saved_state['system_net_charge']
            except KeyError:
                net_charge = 0
            return net_charge
        ...
        if load_from is not None:
            ...
            restricted_scheme = saved_state['restricted_scheme']
            system_net_charge = check_net_charge()
        ...
        self.Zmax = Zmax
        self.restricted_scheme = restricted_scheme
        self.system_net_charge = system_net_charge
        
        self.downstream_mode = False
        self.purification_iterations = 3 # A good default
        ...
        for i in range(len(self.orbitals)):
           ...
           # print("Zl so far: ", Zl)
           #print(f" self.orbitals[{i}][0][0] appended to Zl ", Zl)      
            for z, l in self.orbitals[i]:
                self.Norb += 2*l+1
           #     print("self.Norb for far ", self.Norb)
        ...
        self.batch_trace=lambda X: torch.einsum("bii->b", X)
          ################################ZURIEL´s MODIFICATION############################################################
        ...
        #self.analytic_layer = AnalyticLayer(num_electrons =  torch.sum(Z).item() - self.system_net_charge, restricted_scheme = self.restricted_scheme, ) DELETE IF THE DEFINITION INSIDE FORWARD WORKS
    ...
    def save(self, PATH):
        torch.save({
            ...
            'Zmax': self.Zmax,
            "restricted_scheme": self.restricted_scheme,
            "system_net_charge": self.system_net_charge
        }, PATH)
    ...
    def forward(self, R, S = None): 
        ...
        if self.calculate_density_matrix:
        ...
        if self.calculate_density_matrix:
            self.analytic_layer = AnalyticLayer(num_electrons =  torch.sum(self.Z).item() - self.system_net_charge, restricted_scheme = self.restricted_scheme, purification_iterations = self.purification_iterations)
            density_matrix = torch.cat(full_matrix_rows,-2)
            density_matrix =self.analytic_layer(density_matrix, S, self.downstream_mode)
        ...
        if self.calculate_energy:
            ...
            try:
                if self.calculate_overlap_matrix: #solve Roothan equations FC=SCe
                    ...
                    Xt = X.transpose(-1,-2)
                    Fs = torch.bmm(torch.bmm(Xt,density_matrix),X)
                    ...
                    orbital_energies, orbital_coefficients = torch.symeig(density_matrix, eigenvectors=True)
            ...
            if symeig_success:
                ...
                orbital_energies = torch.zeros_like(torch.diagonal(density_matrix, dim1=-2, dim2=-1))
                orbital_coefficients = torch.zeros_like(density_matrix)
        ...
        results['density_matrix'] = density_matrix
