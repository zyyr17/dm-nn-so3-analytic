...
class NeuralNetwork(nn.Module):
    def __init__(self, 
        ...
        #self.analytic_layer = AnalyticLayer(num_electrons =  torch.sum(Z).item(), restricted_scheme = self.restricted_scheme, ) DELETE IF THE DEFINITION INSIDE FORWARD WORKS
    ...
    def forward(self, R, S = None): 
        ...
        if self.calculate_density_matrix:
            self.analytic_layer = AnalyticLayer(num_electrons =  torch.sum(self.Z).item(), restricted_scheme = self.restricted_scheme, purification_iterations = self.purification_iterations)
            density_matrix = torch.cat(full_matrix_rows,-2)
            density_matrix =self.analytic_layer(density_matrix, S, self.purify)
