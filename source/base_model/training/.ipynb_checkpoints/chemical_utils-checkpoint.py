...
chemical_symbols = chemical_symbols_dict[mol_sym]
...
def scf_iterations(preds, positions, Norb):
    ...
    for mol, dm in zip(mols, preds):
        mf = dft.RKS(mol).density_fit(auxbasis='def2-svp-jkfit')  # RI-J like ORCA
        mf.xc = 'PBE'
...
def transform(hamiltonians,convention, atoms = atoms_args):
