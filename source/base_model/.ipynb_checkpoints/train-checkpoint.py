...
Norb = model.Norb
num_electrons = torch.sum(model.Z)
restricted = model.restricted_scheme
restricted_scheme = model.restricted_scheme
...
transition_epochs = 2500

#(DFTDIFF, PSD, TRE, IDE)
initial_weights =  [1.00, 0.0, 0.0, 0.0]
second_weights = [1.00, 0.0, 0.0, 0.0]
third_weights = [1.0, 0.00, 0.0, 0.0]
...
beta_scheduler = BetaScheduler(
                      ...
                      schedules=[
                                          smooth_transition_schedule(0, transition_epochs),  
                                          smooth_transition_schedule(0, transition_epochs),
                                          smooth_transition_schedule(0, transition_epochs),
                                          smooth_transition_schedule(0, transition_epochs)
...
analytic_correction_lr_offset = 1e-4
scf_iter_log_lr = 5e-5
...
while step < args.max_steps+1:
    ...
    try:
        ...
        if (not model.purify and not force_trace) and optimizer.param_groups[0]['lr'] <= analytic_correction_lr_offset:
            force_trace = True
            model.purify = True
            print("Applying purification and trace correction from now on!!!")
            print("Now analytically correcting trace!!!")
    ...
    predictions= model(R=data['positions'], S = S if force_trace else None)
    ...
    errors, train_eigvals = compute_error_dict(predictions, data, loss_weights, max_errors, beta, num_electrons, restricted)
        ...
        for valid_batch_num, data in enumerate(valid_data_loader):
            ...
            if "density_matrix" in predictions:
                ...
                valid_predicted_dms_min, valid_predicted_dms_max = torch.min(valid_predicted_dms).item(),  torch.max(valid_predicted_dms).item()
                 #compute error metrics
            errors, valid_eigvals = compute_error_dict(predictions, data, loss_weights, max_errors, beta, num_electrons, restricted)
...
# Print training progress
        ...
        if scf_evals_started and scf_evals_active:
            P_dfts1 = (1.0*predictions["density_matrix"]).detach()
...
def predict_batch(position_list, overlap_matrix_list, model=model, keys=keys):
    """
    ...
    with torch.no_grad():
        ...
        preds = model(R=R, S=S)
        pred_dict = {
            k: [transform(preds[k][i].cpu().numpy(), convention = "phipy") for i in range(len(position_list))]
            for k in keys
        }
    return pred_dict
...
useful_data = {
    ...
    "predicted": {
        "train": [predict_for_indices(train_indices)[k] for k in keys],
        "valid":  [predict_for_indices(valid_indices)[k] for k in keys],
        "test":  [predict_for_indices(test_indices)[k]  for k in keys]
    },
...
if True:
    ...
    dms_valid = useful_data["predicted"]["valid"][0]
